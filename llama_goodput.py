...
# is modeled as a constant term plus a retrogression term which increases
# linearly with the total number of replicas.
LlamaPerfParams = collections.namedtuple("PerfParams", [
...
class LlamaGoodputFunction(object):
    ...
    def throughput(self, num_nodes, num_replicas, atomic_bsz, accum_steps):
        ...
        # accum_time = _llama_predict_accum_time(self._perf_params, atomic_bsz, flop_ratio=flop_info[frozen_layer])
        accum_time = _llama_predict_accum_time(self._perf_params, atomic_bsz, flop_ratio=flop_info[frozen_layer], 
                                        param_ratio=param_info[frozen_layer], num_nodes=num_nodes, num_replicas=num_replicas)
        network_time = _llama_predict_network_time(self._perf_params,
                                             num_nodes, num_replicas, param_ratio=param_info[frozen_layer])
        optim_time = np.exp(_llama_predict_log_optim_time(self._perf_params,
...
def llama_fit_perf_params(num_nodes, num_replicas, atomic_bsz,
    ...
    params = [1e-1, 1e-2] * 3 + [1.0 + 1e-3] + [1e-5, 1.0+1e-3] + [1e-5, 1.0+1e-3] + [0.88]
    if init_params is not None: 
        params = list() 
        for key in ["alpha_c", "beta_c", "alpha_n", "beta_n", "alpha_r", "beta_r", "gamma", "alpha_f", "flop_gamma", "alpha_p", "param_gamma", "beta_cs"]: 
    ...
    upper = [np.inf, np.inf] * 3 + [10.0] + [np.inf, 2.0] + [np.inf, 2.0] + [1.0]
    ...
    grad_fn = autograd.grad(_llama_obj_fn)
    result = scipy.optimize.minimize(_llama_obj_fn, params, args=args,
    ...
    return LlamaPerfParams(*params)


def _llama_rmse(pred, true):
    return np.sqrt((((pred - true)) ** 2).mean())


def _llama_obj_fn(params, num_nodes, num_replicas, atomic_bsz,
            accum_step_time, optim_step_time, flop_ratios, param_ratios):
    params = LlamaPerfParams(*params)
    pred_accum = _llama_predict_accum_time(params, atomic_bsz, flop_ratios, param_ratios, num_nodes, num_replicas) 
    pred_network = _llama_predict_network_time(params, num_nodes, num_replicas, param_ratios) 
    pred_log_optim = _llama_predict_log_optim_time(params, pred_accum, pred_network, flop_ratios, param_ratios)
    
    # RMSLError of accum step time predictions.
    err1 = _llama_rmse(np.log(pred_accum), np.log(accum_step_time))
    # RMSLError of optim step time predictions.
    err2 = _llama_rmse(pred_log_optim, np.log(optim_step_time))
...
def _llama_predict_accum_time(params, atomic_bsz, flop_ratio, param_ratio, num_nodes, num_replicas): 
    params = LlamaPerfParams(*params)
    compute_overhead = params.beta_c * atomic_bsz * (flop_ratio ** params.flop_gamma)
    conds = [num_nodes > 1, num_replicas > 1]
    communication_overhead = np.select(conds, [0.22351741790771484, 0.088697388058617], 1e-8)
    return np.maximum(params.alpha_c + params.beta_cs *  (compute_overhead  + (param_ratio + 1) * communication_overhead), 1e-8)

def _llama_predict_log_optim_time(params, accum_time, network_time, flop_ratio, param_ratio):
    return np.log(accum_time+network_time)


def _llama_predict_network_time(params, num_nodes, num_replicas, param_ratio):
    params = LlamaPerfParams(*params)
    ...
    retrogress = retrogress * np.maximum(num_replicas - 2, 1e-8) * (param_ratio ** params.param_gamma)
    init_overhead = (param_ratio ** params.param_gamma) * params.alpha_p
    return np.maximum((bottleneck + retrogress + init_overhead), 1e-8)


class LlamaFrozenGoodputFunction(object):
    ...
    def evaluate(self, num_nodes, num_replicas, atomic_bsz, accum_steps, frozen_set):
        batch_size = num_replicas * atomic_bsz * (accum_steps + 1)
        if not np.all(self._init_batch_size <= batch_size): 
            import pdb; pdb.set_trace() 
