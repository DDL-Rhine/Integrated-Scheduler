...
class FrozenSpeedupFunction(object):

    def __init__(self, goodput_fn, gpu_utilization_fn, fixed_batch_size=None, max_batch_size=None, atomic_bsz_range=None,
                 accumulation=False, mem_size=32, elastic=None):
        self._gpu_utilization_fn = gpu_utilization_fn
        self._goodput_fn = goodput_fn
        ...
        if self._elastic == 'layer' and self._fixed_batch_size is not None: 
            max_batch_size = self._fixed_batch_size
            
        self._base_goodput, _, _, _ = goodput_fn.optimize(
            num_nodes=1, num_replicas=1, max_batch_size=max_batch_size,
            atomic_bsz_range=atomic_bsz_range, accumulation=accumulation)
    ...
    def __call__(self, num_nodes, num_replicas):
        ...
        if np.count_nonzero(missing) > 0:
            ...
            if self._elastic in ["static"]: 
                max_batch_size = self._fixed_batch_size 
            elif self._elastic in ["layer", "batch"]: 
                if self._elastic == 'layer' and self._fixed_batch_size is not None: 
                    max_batch_size = self._fixed_batch_size
                else: 
                    max_batch_size = self._max_batch_size 
            ...
            goodput, _, _, _ = self._goodput_fn.optimize(
                num_nodes, num_replicas,
